...
//            Log.d("ERROR","PULLING IN DATA " );
  ...
  //          Log.d("ERROR","Pushing out data"+msg.obj.toString().length());
    ...
    //    Log.d("ERROR","Data String : "+data.length());
      ...
      //  Log.d("ERROR","READ THE DATA IN PLUUUUUU");
        if(row.type == 0){
         ...
         //   Log.d("ERROR"," ************** length of string in row "+  row.mMsg.length());
        ...
        //    Toast.makeText(getBaseContext(),"MESSAGE RECEIVED",Toast.LENGTH_LONG).show();
            Log.d("ERROR"," image RECEived   "+row.toString());
            //showNotification(row);
            //showInActivity(row);
          //  byte[] strBytes = new byte[1024 * 1024];
            try {
               ...
               // ByteArrayOutputStream fo= new ByteArrayOutputStream();
                ...
                //Log.d("RRR","image received path :" + row.path);
            ...
            //          fos.write();
                Log.d("ERROR"," image size "+  row.mMsg.length());
    ...
    private void onPushOutData(String data){
        ...
        Log.d("ERROR","Row Size :  " + data.length());
    ...
    public void showNotification(MessageRow row) {
        ...
        if(row.type == 0){
            notification = new Notification(R.drawable.ic_action_discover, row.mMsg, System.currentTimeMillis());
            notification.defaults |= Notification.DEFAULT_VIBRATE;
        }else if(row.type == 1){
            notification = new Notification(R.drawable.ic_action_discover, "image received", System.currentTimeMillis());
            notification.defaults |= Notification.DEFAULT_VIBRATE;
        }
