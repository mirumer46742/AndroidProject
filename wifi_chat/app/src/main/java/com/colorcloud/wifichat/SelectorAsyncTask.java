...
public class SelectorAsyncTask extends AsyncTask<Void, Void, Void>{
	...
	public void processSelectionKey(Selector selector, SelectionKey selKey) throws IOException {
        ...
        //    sChannel.socket().setReceiveBufferSize(1024 * 1024);
    ...
    //    bytes = new byte[1024 * 1024 * 10];
       ...
       //            buf.get(bytes);
                ...
                //   buf.get(bytes);
                   Log.d("ERROR","#*#*#*#*#*#* DATA RECEIVED INSIDE BUFFER *#*#*#*#*#*#");
                //   Log.d("ERROR","buffer received size  : " + buf.limit());
                //   Log.d("ERROR","bytes received size  : " + bytes.length);

//                   jsonString = new String(bytes);  // convert byte[] back to string.
  //                 Log.d("ERROR"," Row Size  "+jsonString.length());
//
  //                 return jsonString;
           ...
           //        count++;
                    ...
                    int x = buf.limit();
                    if(x == 4){
                        Log.d("ERROR","******** INSIDE SIZE *************");
                        size = buf.getInt();
                        Log.d("ERROR","tota size to be received : "+size);
                        ...
                        bytes = new byte[buf.limit()];
                        buf.get(bytes);
                        Log.d("ERROR"," ************************ MESSAGE READ ***************** ");
                        Log.d("ERROR","ByteBuffer : "+buf.limit());
                        Log.d("ERROR","bytes : " +bytes.length);
                        ...
                        bytes = new byte[buf.limit()];
                        buf.get(bytes);
                        Log.d("ERROR","PART RECEIVED : "+bytes.length);
                        //byte[] temp = new byte[totalBytes.length];
                        Log.d("ERROR","size : "+size +" total received :" +totalBytes.length);

                        byte temp[] = new byte[totalBytes.length + bytes.length];
                       // Log.d("ERROR","size : "+size +" total received :" +totalBytes.length);

                        System.arraycopy(totalBytes,0,temp,0,totalBytes.length);
                      //  Log.d("ERROR","copy 1 size : "+size +" total received :" +totalBytes.length);

                        System.arraycopy(bytes,0,temp,totalBytes.length , bytes.length);
                      //  Log.d("ERROR","copy 2 size : "+size +" total received :" +totalBytes.length);

                        totalBytes = new byte[temp.length];

                        System.arraycopy(temp,0,totalBytes,0, temp.length);
                        Log.d("ERROR","copied 3 size : "+size +" total received :" +totalBytes.length);

                        if( totalBytes.length  == size){

                            Log.d("ERROR","RECEIVED TOTAL FILE IN CHENKS");
                            size=0;

                            jsonString = new String(totalBytes);  // convert byte[] back to string.
                            Log.d("ERROR"," Row Size  "+jsonString.length());
