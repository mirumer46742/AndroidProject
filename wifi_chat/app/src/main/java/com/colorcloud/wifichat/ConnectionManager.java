...
public class ConnectionManager {
	...
	public static ServerSocketChannel createServerSocketChannel(int port) throws IOException {
      ...
      //  ssChannel.socket().setReceiveBufferSize(1024 * 1024);
	...
	public int startClientSelector(String host) {
		...
		try {
           ...
           // mClientSocketChannel.socket().setReceiveBufferSize(5000);
	...
	public int startServerSelector() {
		...
		try {			
		   ...
		   // mServerSocketChannel.socket().setReceiveBufferSize(1024 * 1024);
	...
	private int writeData(SocketChannel sChannel, String jsonString){
       ...
       // bytebuf.
		...
		try {
            ...
            Log.d("ERROR","to be written total buf : "+ bytebuf.limit());
            Log.d("ERROR","to be written total byte : "+ buf.length);

          //  Log.d("ERROR", "Bytes : " + buf.length);
          //  Log.d("ERROR", "Bytesbuffer  : " + bytebuf.limit());
          //  nwritten = sChannel.write(bytebuf);

          //  Log.d("ERROR","WRITTEN size : "+nwritten );
            ...
            if(size < 5000) {
                ...
                buff.putInt(size);
                Log.d("ERROR", "*****### ***** " + buff.limit());
                buff.flip();
                nwritten = sChannel.write(buff);
                Log.d("ERROR", "WRITTEN size : " + nwritten);
               ...
               /*
                ...
                while (true) {
                    ...
                    Log.d("ERROR", "Limi : " + lim + " Max : " + max + " Size : " + size);
                    byte[] tempBytes = new byte[max];

                    System.arraycopy(buf, lim, tempBytes, 0, max);

                    Log.d("ERROR", "byte Buf size " + tempBytes.length);
                    ByteBuffer bb = ByteBuffer.allocate(tempBytes.length);
                    bb.put(tempBytes);
                    bb.flip();

                    lim = lim + max;
                    size = size - max;
                    max = size < 5000 ? size : 5000;

                    nwritten = sChannel.write(bb);

                    Log.d("ERROR", "remaining   Limi : " + lim + " Max : " + max + " Size : " + size);
                    Log.d("ERROR", "WRITTEN " + nwritten);
                    if (size <= 0) {
                        break;
                    }
